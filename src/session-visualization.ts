...
<body>
  ...
  <script>
    ...
    const sessionLabel=session=>session.depth?((session.meta&&session.meta.agentRole)||session.harness)+' · '+session.sessionId.slice(0,8):(session.title||session.sessionId.slice(0,10));
    ...
    function renderGraph(){
      const query=$('search').value.toLowerCase();const activeEvents=events.filter(event=>event.ts<=selectedTs);const activeMessages=messages.filter(message=>message.ts<=selectedTs);const visibleFiles=files.filter(file=>file.path.toLowerCase().includes(query)&&file.changes.some(change=>change.ts<=selectedTs)).sort((left,right)=>right.changes[right.changes.length-1].ts-left.changes[left.changes.length-1].ts).slice(0,160);
      ...
      if(vertical){sessions.slice(0,laneCount).forEach((session,index)=>{const x=labelW+38+index*laneH;out+='<line class="lane" x1="'+x+'" y1="55" x2="'+x+'" y2="'+(55+timelineHeight)+'"/><text class="lane-label" x="'+x+'" y="28" transform="rotate(-55 '+x+' 28)">'+esc(sessionLabel(session))+'</text>'})}else{sessions.forEach((session,index)=>{const y=laneY(index);out+='<line class="lane" x1="'+labelW+'" y1="'+y+'" x2="'+(labelW+plotW)+'" y2="'+y+'"/><text class="lane-label" x="'+(10+session.depth*10)+'" y="'+(y+4)+'"><title>'+esc(session.title||session.sessionId)+'</title>'+(session.depth?'↳ ':'')+esc(sessionLabel(session))+'</text>'})}
      ...
      if(!lineMode){visibleFiles.forEach((file,index)=>{const x=vertical?labelW+12+(index%fileCols)*(fileW+20):labelW+12+(index%fileCols)*(fileW+20);const y=vertical?timelineHeight+55+Math.floor(index/fileCols)*fileH:fileBase+Math.floor(index/fileCols)*fileH;const changes=file.changes.filter(change=>change.ts<=selectedTs);const latest=changes.at(-1);if(latest){const p=eventPos.get(latest.event);if(p)out+='<line class="edge" x1="'+p[0]+'" y1="'+p[1]+'" x2="'+(x+fileW/2)+'" y2="'+(y+13)+'"/>'}out+='<g class="file-node '+(selectedFile===file.id?'selected':'')+'" data-file="'+esc(file.id)+'" transform="translate('+x+','+y+')"><rect width="'+fileW+'" height="27" rx="6"/><text x="9" y="18">'+esc(shortPath(file.path))+'</text><title>'+esc(file.path)+'</title></g>'})}else{visibleFiles.forEach(file=>file.changes.filter(change=>change.ts<=selectedTs&&file.path.toLowerCase().includes(query)).forEach(change=>{const p=eventPos.get(change.event);if(!p)return;const x=vertical?p[0]:p[0],y=vertical?p[1]:p[1]+30;out+='<circle class="event" data-file="'+esc(file.id)+'" cx="'+x+'" cy="'+y+'" r="4" fill="'+COLORS.files+'"><title>'+esc(file.path)+'</title></circle>'}))}
      ...
      $('graph').setAttribute('viewBox','0 0 '+width+' '+height);$('graph').style.height=Math.max(260,height)+'px';$('graph').innerHTML=out;setSummary(activeMessages.length,activeEvents.length,visibleFiles.length+(files.length>visibleFiles.length?' (cap 160)':''));
