...
export type AutopilotHarness = "codex" | "opencode" | "hermes";
...
function normalizeTarget(target: { harness: AutopilotHarness; sessionId: string; cwd: string }) {
  if (!(["codex", "opencode", "hermes"] as string[]).includes(target.harness)) throw new Error("unsupported autopilot harness");
